    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            if (from != null && to != null) {
                val newFromAmount = from.amount - amount
                val newToAmount = to.amount + amount
                when {
                    newFromAmount < 0 -> errorMessage = "Underflow"
                    newToAmount > MAX_AMOUNT -> errorMessage = "Overflow"
                    else -> {
                        from.newAmount = newFromAmount
                        to.newAmount = newToAmount
                    }
                }
                completed = true
            }
